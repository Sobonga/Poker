...
public class Graph {
    ...
    private int calculateNoOfNodes(Routes[] routes) {
        int noOfNodes = 0;
        for (Routes r : routes) {
            if (r.getPlanet_destination ().indexOf(0) > noOfNodes)
                noOfNodes = r.getPlanet_destination ().indexOf(0);
            if (r.getPlanet_destination ().indexOf(0) > noOfNodes)
                noOfNodes = r.getPlanet_destination ().indexOf(0);
        }
        noOfNodes++;
    ...
    public void calculateShortestDistances() {
        ...
        for (int i = 0; i < this.nodes.length; i++) {
            ...
            ArrayList<Routes> currentNodeRoutes = this.nodes[nextNode].getRoutes();
           /* for (int joinedEdge = 0; joinedEdge < currentNodeRoutes.size(); joinedEdge++) {
                int neighbourIndex = currentNodeRoutes.get(joinedEdge).getNeighbourIndex(nextNode.);
                // only if not visited
               *//* if (!this.nodes[neighbourIndex].isVisited()) {
                    int tentative = this.nodes[nextNode].getDistance_from_source () + currentNodeRoutes.get(joinedEdge).getLength();
    ...
    public void printResult() {
        ...
        output += "\nNumber of edges = " + this.no_of_routes;
        for (int i = 0; i < this.nodes.length; i++) {
